...
docs/generate-swagger.json:
	GOROOT=$(shell go env GOROOT) tools/swagger_$(GLIDE_GOOS) generate spec -b ./pkg/server | sed 's@"$$ref": "\#/definitions/cluster",@"schema": {"$$ref": "\#/definitions/cluster"},@g'

docs/update-swagger.json: 
	GOROOT=$(shell go env GOROOT) tools/swagger_$(GLIDE_GOOS) generate spec -b ./pkg/server | sed 's@"$$ref": "\#/definitions/cluster",@"schema": {"$$ref": "\#/definitions/cluster"},@g' > docs/swagger.json
